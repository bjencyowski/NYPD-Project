...
## Refactoring
### Refactoring column type based off the footnotes of the dataset from www.data.cityofnewyork.us 
```{r refactoring}
website
...
# Using the `unique()` command  to find each deomgraphic that don't fit into the established convention
unique(nypd$Perp_age)
unique(nypd$Perp_sex)
unique(nypd$Perp_race)
```
```{r clean perp demo}
# entries in each column that don't match convention. Unkowns are changed to NA for uniformity across the data set
